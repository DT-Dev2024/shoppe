...
import { handlePreventDefault } from "src/helpers";
...
function SearchTrending() {
  ...
  const { searchTrendingMainListInfo: mainListInfo } = useDataSourceContext() as IDataSource;
  const renderMainList = (datas: IDataItem[][], listIndex: number) => {
    if (datas) {
      if (datas[listIndex] !== undefined) {
        return datas[listIndex].map((data, index) => {
          return (
            <a
              key={index}
              href={data.href}
            ...
            >
              <div className="search-trending__main__text">
                <span className="search-trending__main__text__name">{data.productName}</span>
                <span className="search-trending__main__text__description">{data.productDescription}</span>
              </div>
              <img
                src={data.image}
                className="search-trending__img"
                alt=""
              />
            </a>
          );
  ...
  const handleClickViewMoreButton = (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  return (
    <div className="search-trending">
      <div className="search-trending__heading">
        <span className="search-trending__heading__title">XU HƯỚNG TÌM KIẾM</span>
        <button
      ...
      <div className="search-trending__main">
        <div
          ref={mainListRef}
          className="search-trending__main__list"
        >
